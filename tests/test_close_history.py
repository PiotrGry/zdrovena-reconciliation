...
class TestBuildHistoryEntry:
    def test_basic_fields_always_present(self):
        entry = build_history_entry(
            year=2026, month=4, month_name="Kwiecień",
    ...
    def test_steps_completed_from_report(self):
        ...
        entry = build_history_entry(
            year=2026, month=4, month_name="Kwiecień",
            status="success", dry_run=False, report=report,
        )
    ...
    def test_no_report_means_no_step_count(self):
        entry = build_history_entry(
            year=2026, month=4, month_name="Kwiecień",
            status="blocked", dry_run=True, error="Missing files",
        )
    ...
    def test_partial_status_with_warnings(self):
        report = MagicMock()
        report.steps_completed = ["Pre-flight", "Folder structure", "Sales invoices",
                                   "JPK & VAT reports", "Cost invoices", "Bank statement check",
                                   "ZIP archive (dry-run)"]
        ...
        entry = build_history_entry(
            year=2026, month=4, month_name="Kwiecień",
            status="partial", dry_run=False, report=report,
        )
...
class TestAppendCloseHistory:
    ...
    def test_appends_multiple_entries(self):
        storage = _make_storage("")
        for month in [1, 2, 3]:
            append_close_history(storage, build_history_entry(
                year=2026, month=month, month_name=f"M{month}",
                status="success", dry_run=False,
            ))
        records = _read_tmp(storage)

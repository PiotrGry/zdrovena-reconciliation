...
export default function FilesView() {
  ...
  const uploadToInbox = useCallback(
    async (file) => {
  ...
  const deleteInboxFile = useCallback(
    async (key) => {
      ...
      <div className="card">
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 12, color: "var(--text-dim)" }}>
              Faktury dostawców, wyciąg PKO BP, raporty Fakturowni •{" "}
              <code>faktury/inbox/</code>
            </div>
          </div>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <input
              ...
              style={{ display: "none" }}
              onChange={(e) => {
                Array.from(e.target.files).forEach(uploadToInbox);
                e.target.value = "";
              }}
            />
            <button
              className="btn btn-primary btn-sm"
              onClick={() => inboxFileInput.current.click()}
            >
        ...
        <div
          ...
          onDrop={(e) => {
            ...
            Array.from(e.dataTransfer.files).forEach(uploadToInbox);
          }}
        >
          {inboxLoading && (
            <div
          ...
          {!inboxLoading && inboxItems.length === 0 && (
            <div
              className={`dropzone${inboxDragOver ? " active" : ""}`}
              style={{ margin: "8px 16px" }}
            >
              <span className="hint">
                {inboxDragOver
                  ? "Upuść pliki tutaj"
                  : 'Przeciągnij pliki lub kliknij „Wgraj do inbox"'}
              </span>
            </div>
          )}
          {!inboxLoading && inboxItems.length > 0 && (
            <div style={{ overflowX: "auto" }}>
                        ...
                        <td>
                          <div className="row-actions">
                            ...
                            <button
                              ...
                              title="Usuń z inbox"
                              onClick={() => deleteInboxFile(getKey(file))}
                            >
              ...
              >
                <span className="hint">
                  {inboxDragOver
                    ? "Upuść pliki tutaj"
                    : "Upuść pliki aby dodać do inbox"}
                </span>
              </div>
            </div>
          )}

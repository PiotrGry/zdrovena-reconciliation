...
export default function FilesView() {
    ...
    const loadFiles = useCallback(async (pref = prefix) => {
        setLoading(true)
        try {
            const token = await getToken()
            const res = await fetch(`/api/files?prefix=${encodeURIComponent(pref)}`, {
    ...
    const folders = items.filter(i => i.is_directory || i.type === 'folder' || (i.name || i.key || '').endsWith('/'))
    const files = items.filter(i => !(i.is_directory || i.type === 'folder' || (i.name || i.key || '').endsWith('/')))
    ...
    const filtered = files
        .filter(i => {
            const n = getName(i)
            return matchesFilter(n, filter) && (!search || n.toLowerCase().includes(search.toLowerCase()))
        })
        .sort((a, b) => {
            let va, vb
            if (sortKey === 'name') { va = getName(a).toLowerCase(); vb = getName(b).toLowerCase() }
            else if (sortKey === 'size') { va = a.size ?? 0; vb = b.size ?? 0 }
            else { va = a.last_modified ?? ''; vb = b.last_modified ?? '' }
            if (va < vb) return sortAsc ? -1 : 1
            if (va > vb) return sortAsc ? 1 : -1
            return 0
        })

    const SortInd = ({ k }) => sortKey === k ? (
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--gap)' }}>
            ...
            <div className="crumbs">
                ...
                <button className="seg" onClick={() => loadFiles('')}>zdrovena-docs</button>
                {segments.map((seg, i) => {
                    const p = segments.slice(0, i + 1).join('/')
                    return (
                        <>
                            <span key={`sep-${i}`} className="sep">/</span>
                            <button key={`seg-${i}`} className={`seg${i === segments.length - 1 ? ' current' : ''}`}
                                onClick={() => loadFiles(p)}>{seg}</button>
                        </>
                    )
                })}
            </div>
            ...
            <div className="toolbar">
                ...
                <div className="filter-group">
                    {[['all', T.filter_all], ['pdf', T.filter_pdf], ['xml', T.filter_xml], ['csv', T.filter_csv], ['archive', T.filter_archive]].map(([k, label]) => (
                        <button key={k} className={`filter-btn${filter === k ? ' active' : ''}`} onClick={() => setFilter(k)}>
                            {label}
                        </button>
                    ))}
                </div>
                <button className="btn btn-ghost btn-sm" onClick={() => loadFiles(prefix)} title="Odśwież">
            ...
            <div className="card">
                    ...
                    <table className="files">
                        <thead>
                            <tr>
                                ...
                                <th className="sortable" onClick={() => toggleSort('size')} style={{ width: 110 }}>
                                    {T.col_size} <SortInd k="size" />
                                </th>
                                <th className="sortable" onClick={() => toggleSort('modified')} style={{ width: 160 }}>
                        ...
                        <tbody>
                            ...
                            {!loading && folders.map(folder => {
                                ...
                                const subPrefix = prefix ? `${prefix}/${n.replace(/\/$/, '')}` : n.replace(/\/$/, '')
                                return (
                                    <tr key={getKey(folder)}>
                                        <td>
                                            <div className="name-cell folder" onClick={() => loadFiles(subPrefix)}>
                                                <span className="ext-chip fld"><Icon name="folder" size={12} /></span>
                                                <span className="main-text folder-name">{n.replace(/\/$/, '')}</span>
                                            </div>
                                        </td>
                                        <td className="dim">—</td>
                                        <td className="dim">—</td>
                            ...
                            {!loading && filtered.length === 0 && folders.length === 0 && (
                                <tr><td colSpan={4}>
                                    <div className="empty"><div className="h">Brak plików</div><div>Folder jest pusty lub nie pasuje do filtrów.</div></div>
                                </td></tr>
                            )}
                        </tbody>
                    </table>
                ...
                <div
                    ...
                    onDrop={handleDrop}
                >
                    <span className="hint">{dragOver ? T.dropzone_active : T.dropzone_hint}</span>

...
# TEST_MODE: when set (TEST_MODE=1 in env), the sanitizer produces obviously
# fake test values ("TEST Buyer", "+48000000000", "ul. Testowa 1", ...) instead
# of plausibly-real synthetic values. Use for committed fixtures so PII
# sanitization is verifiable at a glance and no reviewer mistakes fixtures for
# real data. Structural fields (product names, method ids, pickup point ids,
# invoice numbers, quantities) are ALWAYS preserved.
TEST_MODE = os.environ.get("TEST_MODE", "").strip() in ("1", "true", "yes", "on")
FIXTURE_DIRS = [
...
def _pick_replacement(field: str, seed_val: str) -> str | None:
    ...
    elif field in ("phoneNumber", "phone_number"):
        return "+48" + fake_phone(seed_val)
...
def _walk(obj, path: tuple[str, ...] = ()):
    if isinstance(obj, dict):
        ...
        return {
            k: (
                _process_note_attributes(v)
                if k == "note_attributes" and isinstance(v, list)
                else _walk_value(k, v, (*path, k), parent)
            )
...
def main():
    if TEST_MODE:
        print("[TEST_MODE=1] emitting uniform test values (TEST Buyer / 00-000 / +48000000000 ...)")
    files = []
